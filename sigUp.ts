...
export class SignalItemComponent implements OnInit {
  ...
  sortDropdownOptions(options: DropdownOptions[]) {
    ...
    this.datasource = this.formEntityData(this.signalItem?.signalEntities).flatMap((ele:any)=>(ele.signalEntityFiles.filter((myfile:any)=>(this.isAllowedExtension(myfile.original_name))).map((myfile:any,index:any)=>({ ...ele,mysignalEntityOriginalFileName:myfile.original_name,mysignalEntityFileName:myfile.file_name,file_number:myfile.file_number,file_path:myfile.file_path,file_id:myfile.id,file_comments:myfile.file_comments}))));
    this.datasource1 = this.formEntityData(this.signalItem?.signalEntities).flatMap((ele:any)=>(ele.signalEntityFiles.filter((myfile:any)=>(!this.isAllowedExtension(myfile.original_name))).map((myfile:any,index:any)=>({ ...ele,mysignalEntityOriginalFileName:myfile.original_name,mysignalEntityFileName:myfile.file_name,file_path:myfile.file_path,file_id:myfile.id,file_comments:myfile.file_comments}))));
  ...
  deleteSignalEntity(item:any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result == 'confirmed') {
        console.log(item);
        this.service.deleteSignalEntity(item.file_id).subscribe((result) => {
  ...
  deleteSignalEntityFile(item:any) {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      width: '40%',
      data: {
        title: 'Delete Event File',
        message: `Event File ${this.signalItem.signal_number}-${item.entity_number}-${item.file_id} will be permanently deleted. Are you sure?`,
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result == 'confirmed') {
        console.log('entityfile' +item);
        this.service.deleteSignalEntityFile(item.file_id).subscribe((result) => {
          
          this.notification.openSnackbar(
            'Event File deleted successfully!!',
            'Close',
            snackbarColors.success
          );
          this.getSignalItemById(this.signalItem.id);
  ...
  //         {
  //           console.log(data1);
  //           console.log(data1.signalEntityFiles);
  //           data1.signalEntityFiles[i].file_comments=data1.file_comments;
